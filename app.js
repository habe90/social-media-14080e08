// SELAMY - MAIN ENTRY (POSTGRESQL INTEGRATION + LIKE FIX + REEL UPLOAD FIX)
...
async function initApp() {
  ...
  if (user) { state.currentUser.fullname = user.full_name || user.fullname; state.currentUser.username = user.nickname || user.username; state.currentUser.email = user.email; if (user.phone) state.currentUser.phone = user.phone; if (user.bio) state.currentUser.bio = user.bio; if (user.avatar) state.currentUser.avatar = user.avatar; updateProfileUI(); }
}

function updateProfileUI() {
  const pu = document.querySelector('.profile-username'); if (pu) pu.innerHTML = `${state.currentUser.username} <i class="fa-solid fa-circle-check verified-badge"></i>`;
  const fn = document.querySelector('.full-name'); if (fn) fn.textContent = state.currentUser.fullname;
  ['edit-fullname','edit-username','edit-email','edit-phone'].forEach(id => { const el = document.getElementById(id); if (el) el.value = state.currentUser[id.replace('edit-','')] || ''; });
}
...
function setupEventListeners() {
  ...
  document.getElementById('btn-open-create')?.addEventListener('click', e => { e.preventDefault(); playSound('click'); openModal(document.getElementById('create-modal')); if (state.currentUser.location) { const pl = document.getElementById('post-location'); if (pl&&!pl.value) pl.value = state.currentUser.location.text; } });
  document.getElementById('btn-mobile-create')?.addEventListener('click', () => { playSound('click'); openModal(document.getElementById('create-modal')); });
  ...
  document.getElementById('btn-edit-profile')?.addEventListener('click', () => { playSound('click'); openModal(document.getElementById('edit-profile-modal')); });
  ...
  document.getElementById('btn-confirm-logout')?.addEventListener('click', () => { playSound('click'); setToken(null); closeAllModals(); if(lo)lo.classList.remove('hidden'); showToast('Odjavljeni ste.'); });
  ...
  document.getElementById('btn-do-login')?.addEventListener('click', async () => {
    ...
    if(!u||!p){showToast('Unesite korisničko ime i lozinku.');return;}
    showToast('Prijavljujem se...'); const r=await loginUser({login:u,password:p});
    if(r?.user){state.currentUser.fullname=r.user.full_name||r.user.fullname;state.currentUser.username=r.user.nickname||r.user.username;state.currentUser.email=r.user.email;if(r.user.phone)state.currentUser.phone=r.user.phone;if(r.user.avatar)state.currentUser.avatar=r.user.avatar;if(r.user.bio)state.currentUser.bio=r.user.bio;updateProfileUI();playSound('notification');if(lo)lo.classList.add('hidden');showToast(`Dobrodošli, @${state.currentUser.username}!`);switchTab('home');}else{showToast(r?.error||'Greška.');}
  });
  ...
  document.getElementById('btn-do-register')?.addEventListener('click', async () => {
    ...
    showToast('Registrujem...'); const r=await registerUser({full_name:fn,nickname:un.replace(/^@/,''),email:em,phone:ph,password:pw});
    if(r?.user){state.currentUser.fullname=r.user.full_name;state.currentUser.username=r.user.nickname;state.currentUser.email=r.user.email;state.currentUser.phone=r.user.phone;updateProfileUI();playSound('notification');if(lo)lo.classList.add('hidden');showToast(`Registracija uspješna! Dobrodošli, @${state.currentUser.username}! 🎉`);switchTab('home');}else{showToast(r?.error||'Greška.');}
  });
}
...
async function handleCreatePost() { if(!state.uploadedMedia.post?.url){showToast('Izaberite sliku.');return;} playSound('post'); const u=state.uploadedMedia.post.url,c=document.getElementById('post-caption').value.trim(),l=document.getElementById('post-location').value.trim()||(state.currentUser.location?.text||'Kalesija (Babajići)'); showToast('Spremam...'); const bp=await createPostAPI({caption:c,image_url:u,location:l}); state.posts.unshift({id:bp?.id||('p-'+Date.now()),author:state.currentUser.username,avatar:state.currentUser.avatar,verified:true,location:l,image:u,caption:c||'Nova objava! ✨',likes:0,saved:false,timeAgo:'UPRAVO SADA',comments:[]}); state.currentUser.postsCount++; renderPosts(); renderProfileGrid(); resetUploadState('post'); closeAllModals(); document.getElementById('post-caption').value='';document.getElementById('post-location').value=''; showToast('Objava sačuvana u bazi!'); switchTab('home'); }
async function handleCreateStory() { if(!state.uploadedMedia.story?.url){showToast('Izaberite sliku.');return;} playSound('post'); const u=state.uploadedMedia.story.url,t=document.getElementById('story-text-input').value.trim(); showToast('Spremam...'); const bs=await createStoryAPI({media_url:u,text:t}); let my=state.stories.find(s=>s.isMe);if(!my){my={id:'my-story',isMe:true,username:'Vaša priča',avatar:state.currentUser.avatar,hasUnseen:true,slides:[]};state.stories.unshift(my);} my.slides.push({id:bs?.id||('slide-'+Date.now()),media:u,time:'Upravo',text:t}); my.hasUnseen=true; renderStories(); resetUploadState('story'); closeAllModals(); document.getElementById('story-text-input').value=''; showToast('Priča sačuvana u bazi!'); }
async function handleCreateReel() { if(!state.uploadedMedia.reel?.url){showToast('Izaberite video ili sliku.');return;} playSound('post'); const u=state.uploadedMedia.reel.url,c=document.getElementById('reel-caption-input').value.trim()||'Novi kratki video! ✨',a=document.getElementById('reel-audio-input').value.trim()||`Zvuk - ${state.currentUser.username}`; showToast('Spremam Reel...'); const br=await createReelAPI({video_url:u,caption:c,audio_title:a}); state.reels.unshift({id:br?.id||('reel-'+Date.now()),author:state.currentUser.username,avatar:state.currentUser.avatar,caption:c,video:u,audio:a,likes:0,comments:[]}); renderReels(); resetUploadState('reel'); closeAllModals(); document.getElementById('reel-caption-input').value='';document.getElementById('reel-audio-input').value=''; showToast('Reel sačuvan u bazi!'); switchTab('reels'); }
